...
library(ggplot2)
library(ggfortify)
setwd("/mnt/data1/Dementia_Mouse/Array/Data_Exploration/")
...
There is one sample that is between the cortex and hippocampus that needs to be removed. This is the sample that Hovarth's group have predicted to be from the cortex although it is labelled from the hippocampus tissue.

```{r fitler data, warning = F, message = F}

pheno <- QCmetrics
pheno <- pheno[which(pheno$wrongTissue == 0),]
betas <- Normalised_Sesame_Betas
...
cell_chip<- as.data.frame(pheno$Chip_ID, rownames(pheno))
...
colnames(cell_genotype)<- "Genotype"
cell_chip<- as.data.frame(pheno_ecx$Chip_ID, rownames(pheno_ecx))
colnames(cell_chip)<- "Chip"
...
colnames(cell_genotype)<- "Genotype"
cell_chip<- as.data.frame(pheno_ecx_J20$Chip_ID, rownames(pheno_ecx_J20))
colnames(cell_chip)<- "Chip"
...
colnames(cell_genotype)<- "Genotype"
cell_chip<- as.data.frame(pheno_ecx_Tg4510$Chip_ID, rownames(pheno_ecx_Tg4510))
colnames(cell_chip)<- "Chip"
...
J20_ECX <- read.csv("J20_ECX_Pathology.csv", stringsAsFactors = F, header = T)
J20_HIP <- read.csv("J20_HIP_Pathology.csv", stringsAsFactors = F, header = T)
Tg4510_ECX <- read.csv("Tg4510_ECX_Pathology.csv", stringsAsFactors = F, header = T)
Tg4510_HIP <- read.csv("Tg4510_HIP_Pathology.csv", stringsAsFactors = F, header = T)

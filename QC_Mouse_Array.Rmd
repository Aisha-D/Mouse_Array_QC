...
# x_pca <- prcomp(t(betasSexProbes), center = T, scale. = T)
# 
# library(factoextra)
# fviz_eig(x_pca)
# 
...
# 
# SexFactor <- factor(AllSamples$Sex, levels = c(unique(AllSamples$Sex)))
# plot(dat.pcax[,1], dat.pcax[,2], col = rainbow(nlevels(SexFactor))[factor(SexFactor)], cex=2,pch='*')
# legend("topleft", levels(factor(SexFactor)),col = rainbow(nlevels(SexFactor)), cex= 1, pch='*')
```
...
#Manifest file
manifest_file_name_sesame <- "HorvathMammal40.CanonicalManifest.3.2019.sesame.csv"
...
idatpath <- "/gpfs/ts0/scratch/and202/Mouse/idatFiles/"
...
Sset_CE <- as.data.frame(openSesame(paste(idatpath, QCmetrics_CE$Basename, sep = ""), 'custom', manifest_sesame_filtered, "beta", nondetection.mask = FALSE, quality.mask = FALSE), rownames="CGid")
RGset_CE <- RGset_species[,QCmetrics_CE$Basename]

QCmetrics_H <- QCmetrics[which(QCmetrics$Tissue == "Hippocampus"),] 
Sset_H <- as.data.frame(openSesame(paste(idatpath, QCmetrics_H$Basename, sep = ""), 'custom', manifest_sesame_filtered, "beta", nondetection.mask = FALSE, quality.mask = FALSE), rownames="CGid")
RGset_H <- RGset_species[,QCmetrics_H$Basename]

QCmetrics_NT <- QCmetrics[which(QCmetrics$Tissue == "nuclei_total"),] 
Sset_NT <- as.data.frame(openSesame(paste(idatpath, QCmetrics_NT$Basename, sep = ""), 'custom', manifest_sesame_filtered, "beta", nondetection.mask = FALSE, quality.mask = FALSE), rownames="CGid")
RGset_NT <- RGset_species[,QCmetrics_NT$Basename]


QCmetrics_NN <- QCmetrics[which(QCmetrics$Tissue == "nuclei_neuN_neg"),] 
Sset_NN <- as.data.frame(openSesame(paste(idatpath, QCmetrics_NN$Basename, sep = ""), 'custom', manifest_sesame_filtered, "beta", nondetection.mask = FALSE, quality.mask = FALSE), rownames="CGid")
RGset_NN <- RGset_species[,QCmetrics_NN$Basename]

QCmetrics_NP <- QCmetrics[which(QCmetrics$Tissue == "nuclei_neuN_pos"),] 
Sset_NP <- as.data.frame(openSesame(paste(idatpath, QCmetrics_NP$Basename, sep = ""), 'custom', manifest_sesame_filtered, "beta", nondetection.mask = FALSE, quality.mask = FALSE), rownames="CGid")
...
# pdf("Mouse Normalisation.pdf", onefile =  TRUE)
# par(mfrow = c(2,1))
...
#save(QCmetrics, Normalised_Sesame_Betas, file = "/gpfs/ts0/scratch/and202/Mouse/Normalised_Data_Sesame.rdat")
